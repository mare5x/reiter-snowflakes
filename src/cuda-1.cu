...
float get_s(float* grid, Hex hex) {
...
float get_u_part(float* grid, Hex hex) {
...
__global__ static
void update_state_kernel(float* grid, float* grid_new) {
    int q = blockIdx.x * blockDim.x + threadIdx.x;
    int r = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ static
void image_kernel(float* grid, unsigned char* img, float hex_edge) {
